...
export function accountView(db, row) {
  return {
    ...
    usage: accountUsage(db, row)
  };
}

function tableCount(db, table, userId) {
  return Number(db.prepare(`SELECT COUNT(*) AS total FROM ${table} WHERE user_id = ?`).get(userId).total);
}

export function assertStoredLimit(db, account, table, limitName) {
  const plan = effectivePlan(account), used = tableCount(db, table, account.id), limit = PLAN_LIMITS[plan][limitName];
  if (used >= limit) {
...
export function listStrategies(db, userId) {
  return db.prepare(`
...
export function createStrategy(db, account, body) {
  assertStoredLimit(db, account, "strategies", "savedStrategies");
  const rules = requireObject(body.rules, "Rules");
  const id = randomUUID(), versionId = randomUUID(), createdAt = timestamp();
  transaction(db, () => {
    db.prepare("INSERT INTO strategies (id, user_id, name, strategy_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)")
    ...
    db.prepare("INSERT INTO strategy_versions (id, strategy_id, version_number, prompt, rules_json, engine_parameters_json, change_summary, created_at) VALUES (?, ?, 1, ?, ?, ?, ?, ?)")
    ...
    recordUsage(db, account.id, "strategy_saved", { strategyId: id });
  });
  return { id, versionId, versionNumber: 1 };
}

export function createStrategyVersion(db, account, strategyId, body) {
  const strategy = db.prepare("SELECT * FROM strategies WHERE id = ? AND user_id = ?").get(strategyId, account.id);
  if (!strategy) return null;
  const rules = requireObject(body.rules, "Rules");
  const previous = db.prepare("SELECT * FROM strategy_versions WHERE strategy_id = ? ORDER BY version_number DESC LIMIT 1").get(strategy.id);
  const id = randomUUID(), createdAt = timestamp(), versionNumber = Number(previous.version_number) + 1;
  transaction(db, () => {
    db.prepare("INSERT INTO strategy_versions (id, strategy_id, version_number, parent_version_id, prompt, rules_json, engine_parameters_json, change_summary, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
    ...
    db.prepare("UPDATE strategies SET name = ?, strategy_type = ?, updated_at = ? WHERE id = ?")
      .run(String(body.name ?? rules.name ?? strategy.name).slice(0, 120), String(rules.strategyType), createdAt, strategy.id);
    recordUsage(db, account.id, "strategy_version_created", { strategyId: strategy.id, versionNumber });
  });
  return { id, strategyId: strategy.id, versionNumber, parentVersionId: previous.id };
}

export function strategyDetail(db, userId, strategyId) {
  const strategy = db.prepare("SELECT * FROM strategies WHERE id = ? AND user_id = ?").get(strategyId, userId);
  if (!strategy) return null;
  const versions = db.prepare("SELECT * FROM strategy_versions WHERE strategy_id = ? ORDER BY version_number DESC").all(strategy.id).map((row) => ({
...
export function resolveOrbBacktest(db, account, body) {
...
export function listPublicReports(db) {
  return db.prepare("SELECT * FROM reports WHERE visibility = 'public' ORDER BY created_at DESC LIMIT 100").all().map(hydrateReport);
}
export function listReports(db, userId) {
  return db.prepare("SELECT * FROM reports WHERE user_id = ? ORDER BY created_at DESC LIMIT 100").all(userId).map(hydrateReport);
}

export function reportDetail(db, account, id) {
  const row = db.prepare("SELECT * FROM reports WHERE id = ? AND (user_id = ? OR visibility = 'public')").get(id, account.id);
  return row ? hydrateReport(row) : null;
}

export function publicReportDetail(db, id) {
  const row = db.prepare("SELECT * FROM reports WHERE id = ? AND visibility = 'public'").get(id);
  return row ? hydrateReport(row) : null;
}

export function updateReportVisibility(db, userId, id, visibility) {
  if (!["private", "public"].includes(visibility)) throw badRequest("Visibility must be private or public.");
  const result = db.prepare("UPDATE reports SET visibility = ? WHERE id = ? AND user_id = ?").run(visibility, id, userId);
  if (!result.changes) return null;
  recordUsage(db, userId, "report_visibility_changed", { reportId: id, visibility });
  return hydrateReport(db.prepare("SELECT * FROM reports WHERE id = ?").get(id));
}

export function adminMetrics(db) {
  const scalar = (sql) => Number(db.prepare(sql).get().total);
  return {
    totalUsers: scalar("SELECT COUNT(*) AS total FROM users"),
    newSignups30d: scalar("SELECT COUNT(*) AS total FROM users WHERE created_at >= datetime('now', '-30 days')"),
    strategies: scalar("SELECT COUNT(*) AS total FROM strategies"),
    reports: scalar("SELECT COUNT(*) AS total FROM reports"),
    failedTests: 0,
    plans: db.prepare("SELECT plan, status, COUNT(*) AS total FROM subscriptions GROUP BY plan, status").all(),
    eventCounts: db.prepare("SELECT event_type, SUM(quantity) AS total FROM usage_events GROUP BY event_type ORDER BY total DESC").all(),
    symbols: db.prepare("SELECT json_extract(metadata_json, '$.symbol') AS symbol, COUNT(*) AS total FROM usage_events WHERE event_type = 'backtest' GROUP BY symbol ORDER BY total DESC LIMIT 10").all(),
    strategiesByType: db.prepare("SELECT strategy_type, COUNT(*) AS total FROM strategies GROUP BY strategy_type ORDER BY total DESC").all()
  };
}
