...
export default function SaaSWorkspace({ initialPage = "workspace" }: { initialPage?: SaaSPage }) {
  ...
  const [dataLabel, setDataLabel] = useState("Automatic market data ready");
  ...
  return <div className="saas-shell">
    <aside className="saas-sidebar">
      ...
      <a className="saas-nav-link" href="#/lab"><FlaskConical size={16} />Rule Engine Lab</a>
...
function WorkspaceWithPaywall(props: WorkspaceProps) {
  ...
  return <>
    <header className="saas-header"><div><p>STRATEGY WORKSPACE</p><h1>{rules.name}</h1><span>{props.versionId ? "Version saved" : "Unsaved draft"} · {cleanLabel(rules.strategyType)} engine</span></div><div><button onClick={props.saveVersion}><Save size={14} />Save version</button><button className="primary" onClick={props.validateStrategy}><Sparkles size={14} />Validate strategy</button></div></header>
    <div className="saas-workspace"><section>
      ...
      <div className="market-data-status"><Database size={16} /><div><strong>Market data is automatic</strong><span>Free plans use research data. Pro/Power/Trial automatically use Databento for premium intraday and futures windows.</span></div><em>{props.dataLabel}</em><details><summary>Use custom CSV instead</summary><input hidden ref={props.fileRef} type="file" accept=".csv" onChange={(event) => props.onFile(event.target.files?.[0])} /><button onClick={props.upload}><Upload size={13} />Upload CSV</button></details></div>
    ...
    </section><aside className="saas-settings"><details className="advanced-settings" open><summary>Test settings & advanced controls</summary><h2>Test settings</h2><p>Every setting becomes part of the cache key.</p>
      ...
      <label>Date range<select value={rules.dateRange} onChange={(event) => setRules({ ...rules, dateRange: event.target.value as StrategyRules["dateRange"] })}><option value="30d">30 days · Free intraday</option><option value="60d">60 days · Research max</option><option value="6m">6 months · Pro Databento</option><option value="1y">1 year · Pro Databento</option><option value="3y">3 years · Pro Databento</option><option value="5y">5 years · Pro Databento</option></select></label>
      ...
      {props.comparison === "single" && <label>Reward / risk<input type="number" min="0.25" max="20" step=".5" value={rules.rewardRisk} onChange={(event) => setRules({ ...rules, rewardRisk: Number(event.target.value) })} /></label>}
      ...
      <div className="field-row"><label>Max trades/day<input type="number" min="1" max="10" step="1" value={rules.maxTradesPerDay} onChange={(event) => setRules({ ...rules, maxTradesPerDay: Number(event.target.value) })} /></label><label>Timezone<input value={rules.timezone} onChange={(event) => setRules({ ...rules, timezone: event.target.value })} /></label></div><Toggle label="Fees" checked={rules.fees} set={(fees) => setRules({ ...rules, fees })} /><Toggle label="Slippage" checked={rules.slippage} set={(slippage) => setRules({ ...rules, slippage })} />
